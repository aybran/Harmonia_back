...
from app.models.appointment import Appointment, Patient, AppointmentStatus
from app.schemas.schemas import ReportsData, ReportStats, FrequencyData, StatusData, RiskAlert
from app.services.ml_service import calculate_patient_risk
...
def generate_reports_data(db: Session, psychologist_id: int) -> ReportsData:
    ...
    if completed_sessions > 0:
        status_data.append(StatusData(name="Concluídas", value=completed_sessions, color="#26B08F"))
    ...
    if patients_with_sessions_count > 0:
        patients_data.append(StatusData(name="Com sessões", value=patients_with_sessions_count, color="#26B08F"))
    ...
    risk_alerts = []
    for patient_risk in high_risk_patients[:5]:  # Máximo de 5 alertas
        try:
            ...
            # Log do erro e continua o processamento
