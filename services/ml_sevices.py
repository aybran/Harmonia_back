...
def _calculate_risk_score(metrics: Dict) -> int:
    ...
    # Normaliza para 60 dias. Se for maior que 60 dias, o fator é 1.0 (máximo impacto)
    ...
    if metrics["frequency_per_month"] < 1: # Menos de 1 consulta por mês
        score += 20
    elif metrics["frequency_per_month"] < 2: # Meenos de 2 consulta por mês
        score += 10
...
def _determine_risk_level(risk_score: int) -> str:
    """"Determine nivel de risco baseado no score"""
    if risk_score >= 70:
...
def _identify_risk_reason(metrics: Dict) -> str:
    ...
    if metrics["cancellation_rate"] > 0.3:
        ...
        reasons.append("Taxa de cancelamento moderada")
    if metrics["frequency_per_month"] < 1:
        reasons.append("Baixa frequência de consultas")
    elif metrics["frequency_per_month"] < 2:
        reasons.append("Frequência de consultas reduzida")
    if metrics["appointments_last_30"] == 0:
        reasons.append("Sem consultas no último mês")
    if not metrics["has_future_appointments"]:
        reasons.append("Sem consultas futuras agendadas")
    if metrics["recent_trend"] < -1:
        reasons.append("Queda significativa na frequência recente")
    elif metrics["recent_trend"] < 0:
        reasons.append("Leve queda na frequência recente")
    # Retorna a principal razão (ou combinação)
    if not reasons:
        return "Padrão de consultas estável"
    elif len(reasons) == 1:
        return reasons[0]
    else:
        return ", ".join(reasons[:2])  # até duas razões principais    
